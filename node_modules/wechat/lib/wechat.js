  ...
  return function (req, res, next) {
    ...
    res.reply = function (content) {
      ...
      if (!req.query.encrypt_type || req.query.encrypt_type === 'raw') {
          ...
          console.log("4");
          var wrap = {};
        wrap.encrypt = handler.cryptor.encrypt(xml);
        wrap.nonce = req.query.nonce;
        wrap.timestamp = req.query.timestamp;
        //wrap.nonce = parseInt((Math.random() * 100000000000), 10);
        //wrap.timestamp = new Date().getTime();
        wrap.signature = handler.cryptor.getSignature(wrap.timestamp, wrap.nonce, wrap.encrypt);
          console.log("------------------------->");
          console.log(wrap.signature);
          var result=encryptWrap(wrap);
          console.log(result);
          res.end(result);
      }
    };
...
var serveEncrypt = function (that, req, res, next, _respond) {
  ...
  if (method === 'GET') {
    ...
    if (signature !== that.cryptor.getSignature(timestamp, nonce, echostr)) {
      ...
      console.log(echostr);
    var result = that.cryptor.decrypt(echostr);
    // TODO 检查appId的正确性
    res.writeHead(200);
    ...
    load(req, function (err, buf) {
      ...
      xml2js.parseString(xml, {trim: true}, function (err, result) {
        ...
        if (signature !== that.cryptor.getSignature(timestamp, nonce, encryptMessage)) {
          res.writeHead(401);
          res.end('Invalid signature');
          return;
        }
        var decrypted = that.cryptor.decrypt(encryptMessage);
